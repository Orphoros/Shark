...
compile:
	@echo "Compiling Shark binary..."
	@GOARCH=amd64 GOOS=darwin go build ${BIN_FLAGS}  -o ${BUILD_DIR}/bin/Darwin/amd64/${BINARY_NAME} ./cmd/bin
	@echo "[DONE]: Darwin AMD64"
	@GOARCH=amd64 GOOS=linux go build ${BIN_FLAGS} -o ${BUILD_DIR}/bin/Linux/amd64/${BINARY_NAME} ./cmd/bin
	@echo "[DONE]: Linux AMD64"
	@GOARCH=amd64 GOOS=windows go build ${BIN_FLAGS} -o ${BUILD_DIR}/bin/Windows/amd64/${BINARY_NAME}.exe ./cmd/bin
	@echo "[DONE]: Windows AMD64"
	@GOARCH=arm64 GOOS=darwin go build ${BIN_FLAGS} -o ${BUILD_DIR}/bin/Darwin/arm64/${BINARY_NAME} ./cmd/bin

...
func TestMake(t *testing.T) {
	t.Run("should get the correct bytecode instruction", func(t *testing.T) {
		tests := []struct {
			...
			{OpTuple, []int{65534}, []byte{byte(OpTuple), 255, 254}},
			{OpTupleDeconstruct, []int{65534}, []byte{byte(OpTupleDeconstruct), 255, 254}},
			...
			{OpRange, []int{}, []byte{byte(OpRange)}},
			{OpSpread, []int{}, []byte{byte(OpSpread)}},
		}
